...
> H's project is similar to [Hermes](/en/archive/0526-hermes-fts5-session-search) — a long-term personal AI Agent deployed on EC2, centered on a memory system (consolidation + memory.md) and prefix-cache optimization. This was H's strongest area, so I probed it the deepest.
...
**My question: Suppose, like [Hermes agent](/en/archive/0526-hermes-fts5-session-search), memory.md is fixed-length and stuffed in full into the system prompt. I give you two choices — ① with a cap (say 3000 tokens) ② can grow without bound. Which do you think works better?**
